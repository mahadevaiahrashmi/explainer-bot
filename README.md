...
| Voice (optional)     | **macOS `say`** (default) · **Piper** (open-source neural) · **eSpeak-NG** (cross-platform) | Pick with `TTS_ENGINE=`. Piper is dramatically better than `say` and fully open-source; setup is a one-time install + a 30 MB voice download. No cloud TTS bills on any path; primary flow is still user-recorded human voice. |
